...
## [Unreleased]
### Added
- **Enhanced Root Note Display**: Updated the Quantization status line to display root note names with 12-EDO approximations when not in 12-EDO, including cent deviations for notes within 5 cents of standard pitch classes. This provides more intuitive feedback when working with microtonal tuning systems, showing "Root G" instead of "Root 14" for example, while falling back to numeric display for notes without close approximations.
- **Smart Scale Detection**: Added intelligent scale detection that automatically identifies when a custom scale matches a predefined scale for the current EDO. The status line now displays the actual scale name (e.g., "Blues (12-EDO)") instead of "Custom" when a match is found, and the corresponding scale menu item shows a checkmark to indicate the current selection. This greatly improves user experience by providing clear feedback about which scale is actually being used.
...
- **Comprehensive Tuning Systems**: Added extensive new tuning systems including Just Intonation (5-limit, 7-limit, 11-limit, 13-limit), Historical Temperaments (meantone, well-temperaments, Pythagorean), World Music tunings (Arabic Maqam, Indian Raga, Indonesian Gamelan, Thai Classical), and Experimental systems (Bohlen-Pierce, Golden Ratio, Fibonacci, Harmonic Series) with appropriate scale presets for each system.
- **12-EDO Scale Equivalents for Multiples of 12**: Added 12-EDO scale equivalents for EDOs that are multiples of 12 (24, 36, 48, 60, 72, 84, 96, 108, 120 EDO). These scales provide familiar 12-EDO patterns (Major, Minor, Pentatonic, Blues, modes, etc.) but with microtonal subdivisions between the standard semitones. This makes it much easier to work with high-EDO systems by starting from familiar 12-EDO scales and then adding microtonal notes or adjusting existing notes. For example, in 48-EDO you can start with a 12-EDO Major scale and then add quarter-tone notes between the standard semitones. Updated the base 12-EDO scales to match the 16 scales available for multiples of 12, adding Augmented and Diminished scales, reordering alphabetically, and standardizing naming conventions for consistency across all EDO systems. Added "(12-EDO)" suffix to all 12-EDO based scales for multiples of 12 (24, 36, 48, 60, 72, 84, 96, 108, 120 EDO) to clearly distinguish them from other scale types and make it obvious that these are 12-EDO derived scales.
- **Comprehensive Chromatic Scale Support**: Added chromatic scale definitions for all EDOs from 1-120 that didn't already have specific scale definitions. This provides a foundation for future scale additions and ensures every EDO has at least one scale option available. The chromatic scales enable all notes in each EDO system, providing maximum flexibility for microtonal exploration and custom scale creation.
...
- **Added More Scales**: Many more scale options available for 12-EDO. Scales for other EDO options planned (WIP).
- **Added Chords**: Added basic implementation of chords under 12-EDO scales menu. To be expanded into it's own section (WIP).
...
- **Unified Vector-Based Scale System**: Completely migrated the entire codebase from the old hybrid bitmask/vector system to a unified vector-based scale system. This eliminates the complexity of maintaining separate 12-EDO and 24-EDO bitmask variables and provides consistent behavior across all EDOs from 1-120. The new system uses `std::vector<uint8_t>` for all custom scale masks, making the code more maintainable and future-proof. Updated all core quantization logic, configuration save/load, scale seeding, custom scale editor, and degree toggle functions to use the unified system. Fixed scale application issues where scales weren't properly applying due to the core files still expecting the old bitmask variables. Added proper root note transformation when seeding scales from presets, ensuring that scale patterns are correctly rotated based on the selected root note. This resolves issues where the degree picker wasn't properly moving the root note and scales weren't actually being applied during quantization.
- **Unified Scale Selection System**: Completely redesigned the scale selection interface to be more intuitive and streamlined. Removed the confusing "Use custom scale" toggle and replaced it with a unified submenu system where all scales (preset and custom) are selected through submenus. Users can now easily select any scale from the appropriate category (12-EDO Scales, 22-EDO Scales, etc.). This eliminates the workflow confusion between preset and custom scales, making it much easier to start with a preset scale and then modify it as needed.
- **Automatic Scale Mask Preservation**: Removed the "rememberCustomScale" option and made scale mask preservation the default behavior. When changing EDO systems (e.g., from 120-EDO to 12-EDO), the current scale mask is automatically resampled to the new EDO using the `resampleMask` function, preserving the musical structure of the scale. For example, a minor pentatonic scale in 120-EDO will remain a minor pentatonic scale when switching to 12-EDO, with notes rounded to the nearest available degrees. This provides a seamless experience when exploring different tuning systems while maintaining the musical intent of the selected scale.
- **Enhanced MOS Presets Menu**: Completely redesigned the MOS (Moment of Symmetry) presets menu to provide comprehensive access to all available MOS scales. The menu now shows each generator as a submenu containing different mode sizes (5-note, 6-note, 7-note, 8-note, 9-note scales) with their L/S patterns displayed. The best generator option is clearly marked with a star (★) for easy identification. This restores the full functionality that was previously available, allowing users to explore all possible MOS scales for any EDO system and understand the mathematical structure of each scale through the pattern notation. Fixed incorrect generators in the curated map for all EDOs 5-120, replacing non-coprime generators with proper coprime generators that generate valid MOS scales with correct L/S patterns. For example, 36-EDO changed from {6,9,12} to {1,5,7,11}, 48-EDO changed from {6,8,12,16} to {1,5,7,11}, and 72-EDO changed from {6,8,9,12,18} to {1,5,7,11}. This ensures all MOS presets display proper mathematical patterns instead of invalid scales with only small steps.
...
- **PolyQuanta rounding nudges honor tuning**: Directional/Ceil/Floor rounding now derives the nudge size from the active tuning step instead of assuming 12-EDO semitones, keeping non-12 scales aligned.
- **Quantizer limit bounding respects scale masks**: When `boundToLimit` clamps voltages at the range edges, the quantizer now searches for the nearest allowed degree inside the window before falling back to the boundary, preventing masked steps from leaking through at the limits.
