...
## [Unreleased]
### Added
- **Scale Converter Tool**: Added a utility program in `/helpers` that converts scale masks between any N-EDO systems using closest pitch matching. Enables accurate bidirectional conversion between any EDO values (e.g., 24-EDO to 53-EDO). Features startup menu for file input or manual entry, error handling, debug logging, and output formatted for ScaleDefs.cpp integration. Added modern web-based GUI with real-time preview, visual progress bars, and one-click actions. Works offline in any browser with no dependencies.
- **Module Template**: Complete template package for creating new VCV Rack 2 modules based on PolyQuanta architecture.

### Changed
- **Simplified EDO Scale Functions**: Replaced 120-case switch statements with elegant arrays of lambda functions. Reduced both functions from 100+ lines to 8 lines while maintaining functionality.
- **Enhanced Root Note Display**: Updated Quantization status to show root note names with 12-EDO approximations and cent deviations. Shows "Root G" instead of "Root 14" for intuitive microtonal feedback.
- **Smart Scale Detection**: Added intelligent scale detection that identifies when custom scales match predefined scales. Status line shows actual scale name instead of "Custom" and menu items display checkmarks for current selection.
...
- **Additional Non-Octave Systems**: Added non-octave systems (Bohlen-Pierce, Golden Ratio, Fibonacci). (Note: non-octave tunings break certain features)
- **12-EDO Scale Equivalents for Multiples of 12**: Added 12-EDO scale equivalents for EDOs that are multiples of 12 (24, 36, 48, 60, 72, 84, 96, 108, 120 EDO) with familiar patterns and microtonal subdivisions. Updated base 12-EDO scales to match 16 available scales, added "(12-EDO)" suffix for clarity.
- **Comprehensive Chromatic Scale Support**: Added chromatic scale definitions for all EDOs from 1-120 without specific scales. Provides foundation for future additions and ensures every EDO has at least one scale option.
...
- **Added More Scales**: Many more scale options available for multiple EDOs (WIP).
- **Added Chords**: Added basic implementation of chords under 12-EDO scales menu. Possibly to be expanded into it's own section (WIP).
...
- **Unified Vector-Based Scale System**: Migrated from hybrid bitmask/vector system to unified vector-based system using `std::vector<uint8_t>` for all scale masks. Eliminates complexity of separate 12-EDO/24-EDO variables and provides consistent behavior across all EDOs 1-120.
- **Unified Scale Selection System**: Redesigned scale selection interface with unified submenu system for all scales. Removed confusing "Use custom scale" toggle and eliminated workflow confusion between preset and custom scales.
- **Automatic Scale Mask Preservation**: Made scale mask preservation the default behavior when changing EDO systems. Scales are automatically resampled to preserve musical structure across different tuning systems.
- **Enhanced MOS Presets Menu**: Redesigned MOS presets menu with comprehensive access to all available scales. Shows generators as submenus with mode sizes and L/S patterns, with best options marked with stars (★). Fixed incorrect generators for all EDOs 5-120 to use proper coprime generators.
